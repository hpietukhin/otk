(** Command ADT: parse argv → command, then rewrite → (exec, args).
...
let has_traceback_flag args =
  List.exists (String.starts_with ~prefix:"--tb") args

let has_quiet_flag args =
  List.exists (fun s -> s = "-q" || s = "--quiet") args
...
(** Rewrite command into (executable, args) with compact flags.
    Pure: no filesystem or env access. *)
let to_exec = function
  | Git (Status, user_args) ->
    ("git", "status" :: "--porcelain" :: "-b" :: user_args)
  | Git (Log, user_args) ->
    let args =
      user_args
